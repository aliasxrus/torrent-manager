...
chmod 777 ~/tm/ut
```
5. Задаём конфигурацию блокировщика, более подробно можно узнать _**[тут](https://github.com/aliasxrus/go-torrent-manager/blob/main/config.yaml)**:
```shell
...
5. Ссылка для доступа к веб интерфейсуЖ
 ```shell
...
6. Для просмотра логов блокировщика введите команду:
```shell
docker logs -f --tail 100 tm
```
Где:
- _**-f**_ - следит за выводом лога, для выхода из данного режима нажмите Control+C
- _**--tail 100**_ - количество строк, отображаемых с конца
7. Для перезапуска блокировщика и клиента выполните команду:
```shell
docker restart tm
```
