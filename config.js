module.exports = {
    ...
    * */
    ...
    // Удаляет активные загрузки
    removeActiveDownloads: false,
...
/*
	...
	*/
	autostop: {
		...
		*/
		...
		maxSize: 0,
		//Минимальный размер (Mb) (0 - выключено)
		minSize: 0,
		//Максимальное соотношение сиды/пиры (0 - выключено)
		seedPeer: 0,
		...
		* При работе под Linux здесь необходимо указывать не точки монтирования, а физические разделы отслеживаемых дисков. Например: disks: ['/dev/sda2'].
		* Список физических разделов и точек их монтирования можно получить, используя консольную команду 'df -h -x tmpfs'
		*/
		disks: ['C:'],
	...
	*/
    qBitTorrent: {
        ...
        rename: false, //Переименование торрента в соотвествии с его содержимым
        scanInterval: 5000,
        sort: false, //Сортирует очередь торрентов по размеру. Самые маленькие в начало очереди. Полезно, если на закачке много торрентов, и "толстые" торренты в начале очереди тормозят всю очередь.
